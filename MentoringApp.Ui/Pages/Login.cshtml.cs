...
public class LoginModel : PageModel
{
	...
	private async void CreateClaimsAndSignIn()
	{
		...
		// Extract role claims from the JWT so the UI can check roles (e.g. Admin nav link)
		...
		if (!string.IsNullOrEmpty(jwt))
		{
			...
			if (handler.CanReadToken(jwt))
			{
				...
				foreach (var claim in token.Claims.Where(c => c.Type == ClaimTypes.Role || c.Type == "role"))
